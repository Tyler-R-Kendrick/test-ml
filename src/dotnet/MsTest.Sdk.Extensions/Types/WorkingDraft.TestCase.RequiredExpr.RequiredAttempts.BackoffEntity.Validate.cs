                ...
                public readonly partial struct BackoffEntity
                {
                    ...
                    public ValidationContext Validate(in ValidationContext validationContext, ValidationLevel level = ValidationLevel.Flag)
                    {
                        ...
                        if (level > ValidationLevel.Basic)
                        {
                            ...
                            result = result.PushSchemaLocation("https://endjin.com/workspaces/test-ml/docs/spec/draft/working-draft.json#/definitions/testCase/properties/act/properties/retry/properties/backoff");
                        }
                            ...
                            ValidationLevel level = ValidationLevel.Flag)
                        {
                            ...
                            static bool StringValidator(ReadOnlySpan<char> input, in Corvus.Json.Validate.ValidationContextWrapper context, out ValidationContext result)
                            {
                                ...
                                if (Pattern.IsMatch(input))
                                {
                                    ...
                                    {
                                        result = result.WithResult(isValid: true, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} matched '^P(T.*)$'");
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} did not match '^P(T.*)$'");
                                    }
                                    else
                                    {
                                        result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), "Validation pattern - The value did not match '^P(T.*)$'");
                                    }
                                }
...
#else
                        private static Regex CreatePattern() => new("^P(T.*)$", RegexOptions.Compiled);
#endif
                    }
                }
            }
